...
async def stop_session(request):
    ...
    try:
        ...
        if avatar_session:
            transport = getattr(avatar_session.opt, 'transport', 'webrtc')

            if transport in ('webrtc', 'rtcpush'):
                rtc_manager = request.app.get("rtc_manager")
                if rtc_manager:
                    logger.info(f"Stop WebRTC session {sessionid}")
                    await rtc_manager.stop_pc(sessionid)
        ...
        logger.exception('stop_session exception:')
        return json_error(str(e))
...
def setup_routes(app):
    ...
    app.router.add_post("/api/admin/stop_session", stop_session)
